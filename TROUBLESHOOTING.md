...
- **The voice keeps playing on my old earbuds after I switch the system output** (while the thinking sound and beeps move to the new device): the spoken voice runs through one long-lived output stream opened at launch on whatever was default then, and it is deliberately never reopened mid-run (a fresh stream per sentence blips on USB and Bluetooth). The thinking sound and beeps are separate one-shot players that pick up the current default every time, which is why only they follow the switch. Three ways it now catches up: it follows automatically when a device **renumbers** or the pinned `speaker_device` moves (checked every sentence); it refreshes the audio system on its own when a reply starts more than `speaker_recheck_idle_s` seconds after the last one (default 10 — this is what catches a plain "I switched to the desk speakers" on a rig where the mic is a separate device, since nothing else notices); and you can force it immediately by saying **"switch audio output"** (spoken or typed, alone) rather than waiting for the next reply. Each costs one faint onset blip on that first sentence. No restart. Set `speaker_recheck_idle_s` to 0 to disable the idle refresh and rely on the verb alone.
