...
DISCIPLINE = (
    ...
    "No markdown, no lists, no emoji, no URLs. Code is the one "
    "exception: when you genuinely must show code or a config "
    "snippet, wrap it in a triple-backtick fence on its own lines — "
    "it renders silently on the screen and is never spoken, so always "
    "add one short spoken sentence saying what it is or does, and keep "
    "every word of prose outside the fence. Say "
