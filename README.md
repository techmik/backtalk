...
- **The voice console.** Session control by voice, so you never go back to the keyboard: "clear the session", "compact the session", "switch to the deep model" / "back to the fast model", "set effort to low" (or medium, high, max; this one saves itself as your default), "usage report", "go hands free" / "push to talk mode" for the microphone, "switch audio output" to move the voice onto the current default output device (hot-swap earbuds and speakers without a restart), "stop asking for permission" / "start asking again" for approvals. Exact phrases, spoken alone. (Credit where due: this grew out of a community member's own build shared in the Discord.)
...
- **The voice follows an output-device switch on its own now.** `"speaker_device"` is the output twin of `mic_device` — pin the voice to a speaker by name so it survives a device renumbering (a dongle or phone joining or leaving reshuffles every index). And whether or not it's pinned, when a reply starts more than `speaker_recheck_idle_s` seconds (default 10) after the last one, the audio system is refreshed first so the voice lands on whatever the OS default is right then — covering a mid-session switch on a split mic/speaker rig, where nothing else notices. The `"switch audio output"` verb is still there for an immediate move without waiting for the next reply. (Credit where due: the renumbering case was worked out with AbesMaster.)
